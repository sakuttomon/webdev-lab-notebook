...
const filterFamily = (characters, house) => {
  ...
  return charactersFromHouse;
};

const reduceHouses = (characters) => {
  // Return an object with the number of characters from each house
  const housePopulationsObj = characters.reduce((acc, curr) => {
    // If house key exists in acc object, increment it, else create it and start with 1 instance count
    acc.hasOwnProperty(curr.house) ? acc[curr.house]++ : (acc[curr.house] = 1);

    return acc;
  }, {});

  return housePopulationsObj;
};
